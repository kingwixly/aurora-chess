...
3. Save to `./backups/aurorachess_YYYYMMDD_HHMMSS.sql.gz`
...
```bash
gunzip -c backups/aurorachess_20260321_120000.sql.gz | \
  docker compose -f deployment/docker-compose.yml exec -T postgres psql -U postgres aurorachess
# or: make restore FILE=backups/aurorachess_20260321_120000.sql.gz
```
...
- Docker volumes are not included — back up `aurorachess-pgdata` and `aurorachess-redis-data` volumes separately if needed
...
0 3 * * * cd /path/to/aurora-chess && ./scripts/backup.sh >> /var/log/aurorachess-backup.log 2>&1
```
