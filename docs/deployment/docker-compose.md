...
AuroraChess provides three Docker Compose configurations in the `deployment/` directory.
...
| Default registry | N/A                     | `ghcr.io/amiwrpremium/aurora-chess` (override via `IMAGE_REGISTRY`) |
...
```bash
# In .env
IMAGE_REGISTRY=registry.gitlab.com/your-username/aurora-chess
```
...
| ----------------- | ---------------------------- | ----------------------- | ----------------------------------- |
| `pgdata`          | `aurorachess-pgdata`          | `aurorachess-dev-pgdata` | PostgreSQL data                     |
| `redis_data`      | `aurorachess-redis-data`      | —                       | Redis AOF persistence (game clocks) |
| `prometheus_data` | `aurorachess-prometheus-data` | —                       | Prometheus metrics                  |
| `loki_data`       | `aurorachess-loki-data`       | —                       | Loki log data                       |
| `grafana_data`    | `aurorachess-grafana-data`    | —                       | Grafana dashboards and settings     |
| `certbot-certs`   | `aurorachess-certbot-certs`   | —                       | Let's Encrypt certificates          |
| `certbot-webroot` | `aurorachess-certbot-webroot` | —                       | ACME challenge webroot              |
