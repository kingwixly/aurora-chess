...
Replace `{owner}/{repo}` with your GitHub username and repository name (e.g., `amiwrpremium/aurora-chess`).
...
| `SITE_URL`        | Main site URL (baked into images)      | `https://aurora-chess.com`          |
| `ADMIN_URL`       | Admin panel URL (baked into web image) | `https://admin.aurora-chess.com`    |
...
gh secret set SITE_URL --body "https://aurora-chess.com"
gh secret set ADMIN_URL --body "https://admin.aurora-chess.com"
```
...
2. **Push** (~30s): Images are pushed to `ghcr.io/{owner}/aurora-chess/{api,web,worker,migrate,admin}:{version}`
...
docker compose -f deployment/docker-compose.cd.yml ps
docker inspect aurorachess-api --format '{{.Config.Image}}'
# Expected: ghcr.io/amiwrpremium/aurora-chess/api:1.1.5
```
...
make restore FILE=backups/aurorachess_20260325_030000.sql.gz
```
...
grep -A5 "socket.io" /opt/aurorachess/deployment/nginx.conf
...
Buildx runs in a separate builder context where locally-built images (like `aurorachess-base`) are not visible. Since we need to build the base image first and then reference it in app Dockerfiles via `FROM aurorachess-base`, plain `docker build` is simpler and avoids builder context issues. Buildx is only needed for multi-platform builds, which aren't required for a single-architecture VPS.
...
```bash
# List image versions
gh api /user/packages/container/aurora-chess%2Fapi/versions \
  --jq '.[].metadata.container.tags'

# Delete a specific version (by version ID)
gh api -X DELETE /user/packages/container/aurora-chess%2Fapi/versions/<version-id>
```
...
AuroraChess also ships a `.gitlab-ci.yml` that mirrors the GitHub Actions pipeline. It works on both gitlab.com and self-hosted GitLab instances (e.g., your own GitLab).
...
```bash
# Add to /opt/aurorachess/.env
echo 'IMAGE_REGISTRY=registry.gitlab.com/your-username/aurora-chess' >> /opt/aurorachess/.env
```
...
git remote add gitlab https://gitlab.com/your-username/aurora-chess.git
...
AuroraChess supports automatic HTTPS via Let's Encrypt Certbot, running as a Docker container alongside Nginx.

### How It Works

- `SITE_DOMAIN` empty → HTTP-only (development/localhost)
- `SITE_DOMAIN=aurorachess.com` → Certbot obtains cert, Nginx serves HTTPS, HTTP redirects to HTTPS
...
```
aurorachess.com        A    203.0.113.50
admin.aurorachess.com      A    203.0.113.50
grafana.aurorachess.com    A    203.0.113.50
```

Wait for DNS propagation (5-60 minutes). Verify: `dig aurorachess.com`

#### 2. Set environment variables

On your VPS, edit `/opt/aurorachess/.env`:

```bash
SITE_DOMAIN=aurorachess.com
CERTBOT_EMAIL=you@example.com
# Optional: include subdomains in the cert
# CERTBOT_DOMAINS=aurorachess.com admin.aurorachess.com grafana.aurorachess.com
SITE_URL=https://aurorachess.com
NEXT_PUBLIC_API_URL=https://aurorachess.com
NODE_ENV=production
```
...
```bash
curl -I https://aurorachess.com
# HTTP/2 200
# strict-transport-security: max-age=31536000; includeSubDomains

curl -I http://aurorachess.com
# HTTP/1.1 301 Moved Permanently
# Location: https://aurorachess.com/
```
...
```bash
# In .env
CERTBOT_DOMAINS=aurorachess.com grafana.aurorachess.com
```
...
Nginx falls back to the HTTP-only configuration. Existing certs are preserved in the `aurorachess-certbot-certs` volume.
...
| Deploy manually      | `ssh deploy@server "cd /opt/aurorachess && IMAGE_TAG=1.2.0 make deploy"`                                    |
| Rollback             | `ssh deploy@server "cd /opt/aurorachess && make rollback IMAGE_TAG=1.1.5"`                                  |
| Check health         | `curl http://your-domain/health`                                                                           |
| View logs            | `ssh deploy@server "cd /opt/aurorachess && docker compose -f deployment/docker-compose.cd.yml logs -f api"` |
| Backup database      | `ssh deploy@server "cd /opt/aurorachess && make backup"`                                                    |
| Check running images | `ssh deploy@server "docker inspect aurorachess-api --format '{{.Config.Image}}'"`                           |
