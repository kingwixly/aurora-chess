...
Optional guide for putting AuroraChess behind Cloudflare for DDoS protection, CDN caching, and free SSL.
...
```bash
mkdir -p /opt/aurorachess/deployment/ssl
# Paste the origin certificate
nano /opt/aurorachess/deployment/ssl/origin.pem
# Paste the private key
nano /opt/aurorachess/deployment/ssl/origin-key.pem
chmod 600 /opt/aurorachess/deployment/ssl/origin-key.pem
```
...
Check **Network → WebSockets** is enabled. Also verify your Nginx config has the WebSocket upgrade headers (it does by default in AuroraChess).
