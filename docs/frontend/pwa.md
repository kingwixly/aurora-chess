...
AuroraChess is a Progressive Web App (PWA) that supports offline play and installability.
...
- **Deferred updates**: Stored in `sessionStorage` (`aurorachess-deferred-update`). On next navigation or page load, `checkDeferredUpdate()` applies the pending reload.
