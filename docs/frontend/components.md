...
Accordion-style time control selector shared by bot and friend play pages. Groups presets by category (Bullet, Blitz, Rapid, Classical) with expand/collapse. Includes standalone "No Time Limit" button and custom time inputs with a "Select" button. Uses `TIME_CONTROL_PRESETS` from `@aurora/chess` as the single source of truth.
...
Loading placeholder components with pulse animation. Live in `packages/ui` — imported as `@aurora/ui`.
