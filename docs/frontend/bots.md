...
AuroraChess features 31 distinct bot personalities ranging from 200 to 3200 Elo, each with unique playstyles, chat messages, reactions, opening preferences, and simulated think times.
...
- **Auto-save:** Game state saved to localStorage every move under `aurorachess-game-{id}`
...
- If sync fails, the game is added to a pending queue (`aurorachess-pending-syncs` in localStorage) and retried on next page load via `retryPendingSyncs()`
