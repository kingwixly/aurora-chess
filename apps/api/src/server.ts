...
import { adminRoutes, adminBlogRoutes } from "./routes/admin.js";
...
async function main() {
  ...
  // Fail loudly at boot rather than at the first sign-in.
  validateEnv();
  ...
  try {
    ...
    // Convert expired punishments into strikes and restore titles whose block
    // has lapsed. Without this the twelve-month window never elapses and an
    // expired punishment penalises someone forever.
    const strikeInterval = startStrikeJob();
