...
import { updatePeakAndAutoTitle, PUBLIC_USER_SELECT, withTitleOrNull } from "./titles.js";
...
async function endGame(
  ...
  if (result !== "ABORTED" && game.white && game.black && !game.isVsBot) {
    ...
    // Auto titles key off peak rating, so this must run after the rating write.
    // Bot games never reach here (guarded by !game.isVsBot above), so auto
    // titles cannot be farmed against Stockfish.
    await Promise.all([
