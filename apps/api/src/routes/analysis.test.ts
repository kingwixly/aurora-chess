...
describe("analysisRoutes", () => {
  it("accepts both spellings of the analysis path", async () => {
    // The client was moved to "analysis" by a spelling sweep while the route
    // stayed "analyze", which silently broke server-side analysis. Both are
    // registered now, and this asserts it stays that way.
    const routes = app.printRoutes({ commonPrefix: false });
    expect(routes).toContain("analysis");
