...
export async function gameRoutes(app: FastifyInstance) {
  ...
  app.post<{
    ...
    "/games/friend",
    {
      // Friend games survive a restriction, which only removes public
      // matchmaking. A blanket hook on this file blocked them too.
      schema: { body: createFriendGameBodySchema },
      preHandler: requireCapability("playFriends"),
    },
    async (request, reply) => {
      ...
      return { game: withGamePlayers(game) };
    }
  );
  ...
  app.get<{ Params: { id: string } }>("/games/:id/pgn", async (request, reply) => {
    ...
    const game = await prisma.game.findUnique({
      where: { id },
      include: {
        white: { select: { username: true, rating: true, ...TITLE_SELECT } },
        black: { select: { username: true, rating: true, ...TITLE_SELECT } },
  ...
  app.post<{
    ...
    "/games/bot",
    {
      // Bots survive everything short of deactivation: a suspension is about
      // the offence, not about withholding chess.
      schema: { body: createBotGameBodySchema },
      preHandler: requireCapability("playBots"),
    },
   ...
   *
   * Without this a refresh mid-game empties the conversation, which reads as
   * messages having been lost. Players only: a spectator has no business
   * reading what two people said to each other.
   */
  app.get<{ Params: { id: string } }>("/games/:id/chat", async (request, reply) => {
    const userId = request.user.userId;
    const game = await prisma.game.findFirst({
      where: { id: request.params.id, OR: [{ whiteId: userId }, { blackId: userId }] },
      select: { id: true },
    });
    if (!game) return apiError(reply, 404, GAME_NOT_FOUND, "Game not found");
    ...
    const authorIds = [...new Set(messages.map((m: { authorId: string }) => m.authorId))];
    const authors = await prisma.user.findMany({
      where: { id: { in: authorIds } },
      select: PUBLIC_USER_SELECT,
    });
    const byId = new Map(authors.map((a: { id: string }) => [a.id, withTitle(a as never)]));

    return {
      messages: messages.map(
        (m: { id: string; body: string; createdAt: Date; authorId: string }) => ({
          id: m.id,
          body: m.body,
          createdAt: m.createdAt,
          author: byId.get(m.authorId) ?? null,
        })
      ),
    };
  });
