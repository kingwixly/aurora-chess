      ...
      {CATEGORY_ORDER.map((cat) => {
        ...
        return (
          <div key={cat}>
            <button
              onClick={() => toggle(cat)}
              className={`w-full flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium transition-colors ${hasSelected ? "bg-night-800 text-white" : "bg-night-800 hover:bg-night-700 text-night-300"}`}
            >
            ...
            <div
              className={`overflow-hidden transition-all duration-200 ${isOpen ? "max-h-[600px] mt-1" : "max-h-0"}`}
            >
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-3 p-3 rounded-lg text-left transition-all ${
                        isSelected
                          ? `ring-2 ${colors.ring} bg-night-800`
                          : "bg-night-800/60 hover:bg-night-800/80"
                      }`}
                    >
                      <BotAvatar name={bot.name} avatar={bot.avatar} size={52} />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`rounded-lg px-1.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wider ${colors.bg} text-white`}
                          >
                            {bot.elo}
                          </span>
                        </div>
