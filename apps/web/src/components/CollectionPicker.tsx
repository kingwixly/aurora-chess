...
export default function CollectionPicker({ gameId, open, onClose }: CollectionPickerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-night-900 rounded-lg p-4 max-w-xs w-full mx-4">
        <h3 className="text-sm font-bold mb-3">Add to Collection</h3>
        {loading ? (
          <p className="text-night-400 text-xs">Loading...</p>
        ) : (
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {collections.map((c) => (
              <label
                key={c.id}
                className="flex items-center gap-2 cursor-pointer text-sm p-1 hover:bg-night-800 rounded"
              >
                <input
                  type="checkbox"
        ...
        <button
          onClick={onClose}
          className="w-full mt-3 py-1.5 bg-night-800 hover:bg-night-700 rounded text-sm transition-colors"
        >
          Done
        </button>
      </div>
