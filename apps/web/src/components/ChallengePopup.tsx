...
export default function ChallengePopup() {
  ...
  return (
    ...
    >
      <div className="bg-night-900 rounded-lg p-6 max-w-sm w-full mx-4">
        <h2 id="challenge-title" className="text-lg font-bold mb-2 text-center">
          Challenge!
        </h2>
        <p className="text-night-300 text-center mb-1">
          <span className="font-semibold">{challenge.challenger.username}</span>{" "}
          <span className="text-night-400">({challenge.challenger.rating})</span>
        </p>
        <p className="text-night-400 text-center text-sm mb-4">
        ...
        <div className="flex gap-3">
          <button
            onClick={decline}
            disabled={loading}
            className="flex-1 py-2 bg-night-800 hover:bg-night-700 disabled:opacity-50 rounded font-medium transition-colors"
          >
            Decline
          </button>
          <button
            onClick={accept}
            disabled={loading}
            className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 rounded font-medium transition-colors"
          >
            Accept
          </button>
        </div>
