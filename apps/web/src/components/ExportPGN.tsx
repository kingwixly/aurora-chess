    ...
    return (
      <div className="relative inline-block">
        <button
          ...
          disabled={loading}
          className="px-2 py-1 bg-night-800 hover:bg-night-700 disabled:opacity-50 rounded text-xs transition-colors"
        >
          {loading ? "..." : "PGN"}
        </button>
        {showMenu && (
          <div className="absolute bottom-full mb-1 right-0 bg-night-900 border border-night-700 rounded shadow-lg z-10 min-w-[100px]">
            <button
              onClick={copyPGN}
              className="block w-full px-3 py-1.5 text-xs text-left hover:bg-night-800 transition-colors"
            >
              Copy
            </button>
            <button
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setShowMenu(!showMenu)}
        disabled={loading}
        className="w-full py-2 bg-night-800 hover:bg-night-700 disabled:opacity-50 rounded text-sm font-medium transition-colors"
      >
        {loading ? "Loading..." : "Export PGN"}
      </button>
      {showMenu && (
        <div className="absolute bottom-full mb-1 left-0 right-0 bg-night-900 border border-night-700 rounded shadow-lg z-10">
          <button
            onClick={copyPGN}
            className="block w-full px-3 py-2 text-sm text-left hover:bg-night-800 transition-colors"
          >
            Copy to clipboard
          </button>
          <button
