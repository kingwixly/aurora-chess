...
export default function OpeningsTable({ openings }: OpeningsTableProps) {
  ...
  return (
    <div className="bg-night-800 rounded-lg p-4">
      <h3 className="text-sm font-semibold text-night-300 mb-3">Top Openings</h3>
          ...
          return (
            <div key={o.eco} className="flex items-center justify-between text-sm">
              <div className="min-w-0 flex-1">
                <span className="text-night-400 font-mono text-xs mr-2">{o.eco}</span>
                <span className="text-night-300 truncate">{o.name}</span>
              </div>
              <div className="flex items-center gap-3 shrink-0 ml-2">
                <span className="text-night-400 text-xs">{total} games</span>
                <span
                  className={`text-xs font-medium ${winRate >= 50 ? "text-emerald-400" : "text-red-400"}`}
                >
                  {winRate}%
                </span>
              </div>
