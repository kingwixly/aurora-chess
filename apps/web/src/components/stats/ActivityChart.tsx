...
export default function ActivityChart({ activity }: ActivityChartProps) {
  ...
  return (
    <div className="bg-night-800 rounded-lg p-4">
      <h3 className="text-sm font-semibold text-night-300 mb-3">Activity (30 days)</h3>
        ...
        {activity.map((a) => {
          ...
          return (
            <div key={a.date} className="flex-1 group relative">
              <div
                className="bg-aurora-cyan hover:bg-aurora-cyan rounded-t transition-colors w-full"
                style={{ height: `${height}%`, minHeight: a.count > 0 ? "4px" : "0" }}
              />
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-night-800 text-xs text-white px-2 py-1 rounded whitespace-nowrap z-10">
