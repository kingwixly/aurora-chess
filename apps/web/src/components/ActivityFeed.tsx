  ...
  return (
    <div className="bg-night-900 rounded-lg p-3">
      <button
        ...
        className="w-full flex items-center justify-between text-sm font-semibold text-night-400 mb-2"
      >
          ...
          if (event.link) {
            return (
              <Link
                key={i}
                href={event.link}
                className="block p-1.5 rounded hover:bg-night-800 transition-colors"
              >
                {content}
              </Link>
            );
      ...
      {events.length > 3 && !expanded && (
        <button
          ...
          className="w-full text-xs text-aurora-cyan hover:underline mt-1"
        >
