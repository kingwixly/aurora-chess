...
const RANK_COLORS = ["bg-emerald-500", "bg-yellow-600", "bg-orange-600"];
...
export default function EngineLines({ lines, fen, loading }: EngineLinesProps) {
  ...
  if (formattedLines.length === 0) {
    ...
    return (
      <div className="bg-night-900 rounded-lg overflow-hidden text-xs font-mono">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="flex items-center gap-2 px-3 py-1.5 border-b border-night-800 last:border-0"
          >
            <span className="w-5 h-5 rounded bg-night-800 animate-pulse shrink-0" />
            <span className="w-12 h-4 rounded bg-night-800 animate-pulse shrink-0" />
            <span className="flex-1 h-4 rounded bg-night-800 animate-pulse" />
          </div>
        ))}
      </div>
    );
        ...
        >
          <span
            className={`w-5 h-5 rounded flex items-center justify-center text-white font-bold text-[10px] shrink-0 ${
              RANK_COLORS[line.rank - 1] || "bg-night-700"
            }`}
          >
            {line.rank}
          </span>
          ...
          <span className="text-night-400 truncate">{line.moves}</span>
        </div>
      ))}
    </div>
  );
