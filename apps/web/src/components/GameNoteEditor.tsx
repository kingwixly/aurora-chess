  ...
  if (compact) {
    ...
    if (!expanded && text) {
      return (
        <button
          onClick={() => setExpanded(true)}
          className="text-xs text-night-400 hover:text-night-300 truncate max-w-[120px] text-left transition-colors"
          title={text}
        >
          {text.slice(0, 30)}
          {text.length > 30 ? "..." : ""}
  ...
  return (
    <div className="bg-night-800 rounded-lg p-3">
      ...
      <textarea
        ...
        maxLength={2000}
        rows={compact ? 2 : 3}
        className="w-full px-2 py-1.5 bg-night-900 border border-night-700 rounded text-sm resize-none focus:outline-none focus:border-aurora-cyan"
      />
    </div>
  );
}
