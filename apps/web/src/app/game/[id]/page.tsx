...
import type { Player, MoveRecord, ClockState, ReactionType } from "@aurora/chess";
import { RESULT_LABELS, TERMINATION_LABELS } from "@aurora/chess";
...
export default function GamePage() {
  ...
  /** Playing, as opposed to watching. */
  const isPlayer = Boolean(user && (user.id === white?.id || user.id === black?.id));
    ...
    <main className="flex flex-col h-[100dvh] lg:h-auto lg:min-h-screen overflow-hidden lg:overflow-auto p-1 lg:p-4">
        ...
        <div className="flex flex-col lg:flex-row gap-1 lg:gap-6 items-start flex-1 min-h-0">
          ...
          <div className="flex flex-col flex-1 min-h-0 min-w-0 w-full justify-center lg:justify-start">
            ...
            <div className="flex items-center justify-between px-1 py-0.5">
              <div className="flex items-center gap-1.5">
                <div className="w-5 h-5 lg:w-8 lg:h-8 bg-night-800 rounded-full flex items-center justify-center text-[10px] lg:text-sm font-bold shrink-0">
            ...
            <div className="lg:hidden flex gap-1.5 justify-center px-1 py-0.5">
              <button
                ...
                className="px-2 py-1 bg-night-800 hover:bg-night-700 rounded text-sm"
                title="Flip board"
              >
                {"\uD83D\uDD04"}
              </button>
                  ...
                  {drawIncoming ? (
                    <>
                      <button
                        ...
                        className="px-2 py-1 bg-emerald-500 hover:bg-emerald-400 rounded text-xs"
                      >
                        Accept
                      </button>
                      <button
                    ...
                    <>
                      <button
                        ...
                        className="px-2 py-1 bg-night-800 hover:bg-night-700 disabled:opacity-50 rounded text-sm"
                        title="Offer draw"
                      >
                        {drawOffered ? "\u2713" : "\uD83E\uDD1D"}
                      </button>
          ...
          <div className="hidden lg:block flex-1 space-y-4 min-w-0">
            ...
            <div className="flex gap-2 justify-center flex-wrap">
              ...
              <button
                onClick={() => setFlipDisplay((f) => !f)}
                className="rounded bg-night-800 px-3 py-1.5 text-sm hover:bg-night-700"
                title="Flip board"
                aria-label="Flip board"
              >
                {/* U+21C5 UPWARDS ARROW LEFTWARDS OF DOWNWARDS ARROW. The old
                    `&updownarrow;` is not a defined HTML entity and rendered
                    as literal text. */}
                &#x21C5;
              </button>
              <button
                ...
                className="px-3 py-1.5 bg-night-800 hover:bg-night-700 rounded text-xs"
                title="Copy FEN"
              >
                {fenCopied ? "\u2713 FEN" : "FEN"}
              </button>
              ...
              <div className="flex gap-2">
                {drawIncoming ? (
                  <>
                    <button
                      onClick={() => setConfirmAcceptDraw(true)}
                      className="flex-1 py-2 bg-emerald-500 hover:bg-emerald-400 rounded text-sm font-medium"
                    >
                      Accept Draw
                    </button>
                    <button
                      onClick={declineDraw}
                      className="flex-1 py-2 bg-night-800 hover:bg-night-700 rounded text-sm font-medium"
                    >
                      Decline
                    </button>
                  </>
                  ...
                  <>
                    <button
                      onClick={() => setConfirmDraw(true)}
                      disabled={drawOffered}
                      className="flex-1 py-2 bg-night-800 hover:bg-night-700 disabled:opacity-50 rounded text-sm font-medium"
                    >
                      {drawOffered ? "Draw Offered" : "Offer Draw"}
                    </button>
                    <button
                      onClick={() => setConfirmResign(true)}
                      className="flex-1 py-2 bg-red-500 hover:bg-red-400 rounded text-sm font-medium"
                    >
                      Resign
                    </button>
                  </>
              ...
              <div className="flex gap-2 justify-center">
                {!isSpectator && (
                  <button
                    ...
                    className="px-4 py-2 bg-aurora-cyan hover:bg-[#3ad2e8] rounded font-medium"
                  >
                    Back to Play
                  </button>
                )}
                <button
                  ...
                  className="px-4 py-2 bg-night-800 hover:bg-night-700 rounded font-medium"
                >
      ...
      {reconnecting && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-40">
          <div className="bg-night-900 rounded-lg p-6 text-center">
            <p className="text-lg font-medium mb-2">Reconnecting...</p>
            <p className="text-night-400 text-sm">Trying to reconnect to the game</p>
          </div>
        </div>
      )}
      ...
      {topPlayer && reporting && (
        <ReportDialog
          targetUsername={topPlayer.username}
          gameId={gameId}
          onClose={() => setReporting(false)}
        />
      )}

      {/* Chat and reporting sit together: the report button is where you are
          when something is said that warrants one. */}
      {/* Players only. A spectator shown a chat box would have every message
          rejected by the server, and there is nobody for them to report as an
          opponent. */}
      {isPlayer && (
        <div className="mx-auto mt-4 w-full max-w-[560px] space-y-2 px-3">
      ...
      {gameOver && (
        <GameOverModal
          gameOver={gameOver}
          // Only claim a side when the viewer is actually playing -- a
          // spectator should see the neutral result, not "You lost".
          playerColor={user?.id === white?.id ? "white" : user?.id === black?.id ? "black" : null}
          gameId={gameId}
