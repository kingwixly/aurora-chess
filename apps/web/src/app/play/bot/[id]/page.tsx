...
import type { BotPersonality, MoveRecord, ThinkTimeContext } from "@aurora/chess";
import { computeThinkTime } from "@aurora/chess";
...
export default function BotGamePage({ params }: { params: { id: string } }) {
  ...
  // Mate distance is tracked separately from the centipawn score: collapsing
  // the two is what made the bar read "+1000.0" for a forced mate.
  ...
  if (!initialized) {
    return (
      <main className="flex items-center justify-center min-h-screen">
        <p className="text-night-400">Loading game...</p>
      </main>
    );
  }
        ...
        <div className="flex flex-col lg:flex-row gap-1 lg:gap-4 items-start flex-1 min-h-0">
          ...
          <div className="flex flex-col flex-1 min-h-0 min-w-0 w-full justify-center lg:justify-start">
            ...
            <div className="flex items-center gap-1.5 px-1 py-0.5">
              <BotAvatar name={bot?.name ?? "Bot"} avatar={bot?.avatar} size={28} />
              ...
              {thinking && (
                <span className="flex gap-0.5 ml-1">
                  <span
                    className="w-1 h-1 bg-aurora-cyan rounded-full animate-bounce text-night-950"
            ...
            {activeSettings.evalBar && (
              <div className="lg:hidden h-3 w-full">
                <EvaluationBar evalCP={evalScore} mate={evalMate} />
              </div>
            )}
            ...
            <div className="flex items-center gap-1.5 px-1 py-0.5">
              <div className="w-5 h-5 lg:w-7 lg:h-7 bg-night-800 rounded-full flex items-center justify-center text-[10px] lg:text-xs font-bold shrink-0">
            ...
            <div className="lg:hidden flex gap-1.5 justify-center px-1 py-0.5">
              <button
                ...
                className="px-2 py-1 bg-night-800 hover:bg-night-700 rounded-lg text-sm"
                title="Flip board"
              >
                {"\uD83D\uDD04"}
              </button>
              {!gameOver && activeSettings.hints && (
                <button
                  ...
                  className="px-2 py-1 bg-aurora-cyan hover:bg-[#3ad2e8] disabled:opacity-50 rounded-lg text-sm text-night-950"
                  title="Hint"
                >
                  {"\uD83D\uDCA1"}
                </button>
              ...
              {!gameOver && activeSettings.takeback && moves.length >= 2 && (
                <button
                  ...
                  disabled={thinking}
                  className="px-2 py-1 bg-yellow-600 hover:bg-yellow-700 disabled:opacity-50 rounded-lg text-sm"
                  title="Takeback"
                >
                  {"\u21A9\uFE0F"}
                </button>
              ...
              {!gameOver && (
                <button
                  ...
                  className="px-2 py-1 bg-red-500 hover:bg-red-400 rounded-lg text-sm"
                  title="Resign"
                >
                  {"\uD83C\uDFF3\uFE0F"}
                </button>
          ...
          <div className="hidden lg:block w-72 space-y-3">
            {openingName && (
              <div className="text-xs text-night-400 text-center px-2 py-1 bg-night-900 rounded-lg">
                {openingName}
              </div>
            )}
            {activeSettings.engine && (
            ...
            <div className="flex gap-2 justify-center flex-wrap">
              <button
                ...
                className="px-3 py-1.5 bg-night-800 hover:bg-night-700 rounded-lg text-sm"
              >
                &laquo;
              </button>
              <button
              ...
              <button
                ...
                className="px-3 py-1.5 bg-night-800 hover:bg-night-700 rounded-lg text-xs"
                title="Copy FEN"
              >
                {fenCopied ? "\u2713 FEN" : "FEN"}
              </button>
            ...
            {!gameOver && (
              <div className="space-y-2">
                <div className="flex gap-2">
                  {activeSettings.hints && (
                    <button
                      onClick={handleHint}
                      disabled={!isMyTurn || !botEngine.ready}
                      className="flex-1 py-2 bg-aurora-cyan hover:bg-[#3ad2e8] disabled:opacity-50 rounded-lg text-sm font-medium text-night-950"
                    >
                  ...
                  {activeSettings.takeback && moves.length >= 2 && (
                    <button
                      onClick={handleTakeback}
                      disabled={thinking}
                      className="flex-1 py-2 bg-yellow-600 hover:bg-yellow-700 disabled:opacity-50 rounded-lg text-sm font-medium"
                    >
                      Takeback
                    </button>
                  )}
                ...
                <button
                  onClick={() => setConfirmResign(true)}
                  className="w-full py-2 bg-red-500 hover:bg-red-400 rounded-lg text-sm font-medium"
                >
                  Resign
                </button>
              </div>
      ...
      {gameOver && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-night-950/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-night-900 ring-1 ring-inset ring-night-700">
            {/* Result colour reads before any text does. */}
            <div
              className={`h-1 w-full ${gameOver.includes("Draw") ? "bg-night-700" : (playerIsWhite && gameOver.includes("White wins")) || (!playerIsWhite && gameOver.includes("Black wins")) ? "bg-emerald-400" : "bg-red-400"}`}
            />
            ...
            <div className="p-6 text-center">
              ...
              {/* The bot gets the last word. This is the payoff for the
                  dialogue system, so it sits directly under their portrait. */}
              {gameOverQuote && (
                <p className="mt-3 text-sm italic text-aurora-cyan">
              ...
              <div className="mt-6 space-y-2">
                <button
                  onClick={rematch}
                  className="w-full rounded-lg bg-aurora-cyan py-3 font-semibold text-night-950 transition-colors hover:bg-[#3ad2e8] font-display"
                >
                  Play {bot ? bot.name : "again"} again
                </button>
                <button
                  onClick={playAgain}
