#!/bin/bash
# Backup AuroraChess PostgreSQL database
...
if [ -n "${BACKUP_ENCRYPTION_KEY:-}" ]; then
  FILENAME="aurorachess_${TIMESTAMP}.sql.gz.enc"
  echo "Backing up database (encrypted) to ${BACKUP_DIR}/${FILENAME}..."
  docker compose --env-file .env -f deployment/docker-compose.cd.yml exec -T postgres pg_dump -U postgres aurorachess \
    | gzip \
    | openssl enc -aes-256-cbc -pbkdf2 -pass "pass:${BACKUP_ENCRYPTION_KEY}" \
    > "${BACKUP_DIR}/${FILENAME}"
  PATTERN="aurorachess_*.sql.gz.enc"
else
  FILENAME="aurorachess_${TIMESTAMP}.sql.gz"
  echo "Backing up database to ${BACKUP_DIR}/${FILENAME}..."
  echo "  (Set BACKUP_ENCRYPTION_KEY to enable encryption)"
  docker compose --env-file .env -f deployment/docker-compose.cd.yml exec -T postgres pg_dump -U postgres aurorachess \
    | gzip \
    > "${BACKUP_DIR}/${FILENAME}"
  PATTERN="aurorachess_*.sql.gz"
fi
