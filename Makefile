...
help: ## Show this help
	@echo ""
	@echo "AuroraChess — Makefile Commands"
	@echo ""
	@echo "Production:"
...
build-base: ## Build the shared base Docker image
	docker build -f deployment/Dockerfile.base -t aurorachess-base .
...
db-migrate: ## Run Prisma migrations (dev compose)
	$(DEV_COMPOSE) exec api pnpm --filter @aurora/api run db:migrate

db-seed: ## Run database seed (dev compose)
	$(DEV_COMPOSE) exec api pnpm --filter @aurora/api run db:seed

seed-demo: ## Populate DB with demo data (10 users, friendships, games, collection)
	$(DEV_COMPOSE) exec api pnpm --filter @aurora/api run db:seed-demo

seed-bots: ## Seed/reseed bot personalities from shared definitions
	$(DEV_COMPOSE) exec api pnpm --filter @aurora/api run db:seed-bots

db-studio: ## Open Prisma Studio (requires: make dev-up)
	DATABASE_URL=postgresql://postgres:$${POSTGRES_PASSWORD}@localhost:5432/aurorachess pnpm --filter @aurora/api exec prisma studio
...
restore: ## Restore database (usage: make restore FILE=backups/aurorachess_XXXX.sql.gz)
	@test -n "$(FILE)" || (echo "Usage: make restore FILE=backups/aurorachess_XXXX.sql.gz" && exit 1)
	gunzip -c $(FILE) | $(PROD_COMPOSE) exec -T postgres psql -U postgres aurorachess
...
clean-all: ## Remove everything: containers, volumes, images
	@echo "This will remove all AuroraChess containers, volumes, and images."
	@sleep 3
...
test-chess: ## Run shared chess package tests
	pnpm --filter @aurora/chess test

test-coverage: ## Run all tests with coverage
	pnpm test:coverage

test-watch: ## Run tests in watch mode
	pnpm --filter @aurora/api test:watch
...
shell-postgres: ## Open psql in Postgres container
	$(DEV_COMPOSE) exec postgres psql -U postgres aurorachess
